...
const services = {
    home: require('../test/services/home.js')
}
...
const start = async props => {
    ...
    state.subscriber.subscribe(`${props.redis.prefix}hres:ch`)
    ...
    koa.use(async ctx => {
        ...
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                ...
                logger.debug('timeout', {status: ctx.status, body: ctx.body, requestId})
                resolve()
            }, props.responseTimeoutSeconds*1000)
            state.pending.set(requestId.toString(), async () => {
                ...
                logger.debug('ok', { status: ctx.status, body: ctx.body, requestId })
                resolve()
            })            
